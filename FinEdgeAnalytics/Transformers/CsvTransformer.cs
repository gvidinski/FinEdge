...
public class CsvTransformer : ITransformer
{
	...
	{
		var csvExtractor = _services.GetRequiredService<CsvExtractor>();
		if (csvExtractor is null)
		{
			throw new Exception(message: "CsvExtractor is not registered in the services collection.");
		}
		...
		var enumerator = csvExtractor.GetTransactions().GetAsyncEnumerator();
	...
	public static List<LoaderDto> ConvertData(List<CsvTransactionDto> transactions, int descriptionLength)
	{
		var result = transactions
			.AsParallel()
			.Select(transaction => new LoaderDto(
				transaction.TransactionId,
				DateTime.Parse(transaction.Date).ToUniversalTime(),
				transaction.Amount,
